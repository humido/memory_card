...
question_list=[]
q1 = Question('Select the name for the programming concept to store some data', 'variable', 'variation', 'variant', 'changing')
q2 = Question('When was the great fire of London?', '1666', '1999', '1048', '666')
q3 = Question('What is the smallest country in the world?', 'Vatican', 'Monaco', 'Luxembourg', 'Andorra')
q4 = Question('Which word do we use in programming to do conditional?', 'if', 'while', 'print', 'input')
question_list.append(q1)
question_list.append(q2)
question_list.append(q3)
question_list.append(q4)

window.cur_question = -1     #start counter
def next_question():
    window.cur_question = window.cur_question + 1
    if window.cur_question >= len(question_list):
        window.cur_question = 0                    #reset counter if we reached the end of the question list
...
btn_OK.clicked.connect(click_OK) # remove the test, we need to check the answer here 
next_question()
